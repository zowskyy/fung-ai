...
# Fung Godot Toolkit - metroidvania_demo sample project.
#
# Loads a real, pre-generated candidate payload (produced by actually
# running `python -m bridge.fung_bridge` against the recipe
# "vertical_metroidvania_cavern" at a tall 56x100 map size - see
# data/README.md) and builds a playable level from it at runtime: decodes
# the rle-v1 grid into a TileMapLayer, places a spawn/exit marker pair,
# and spawns a real (gravity + jump) side-view platformer controller
# (player.gd) at the PlayerSpawn position. Spawn/exit are placed on the
# top/bottom edges by the generator regardless of orientation, which
# lines up naturally with a tall map here.
#
...
func _build_terrain(payload: Dictionary) -> void:
	var grid_size: Array = payload.get("grid_size", [56, 100])
	...
	# with_wall_collision=true: this is the one demo where the player
	# controller needs real terrain collision (see player.gd and this
	# project's README.md for the honest caveat on whether it works).
	tilemap.tile_set = TilesetBuilder.new().build(true)
...
func _build_debug_label(result: Dictionary, payload: Dictionary) -> void:
	...
	label.text = (
		"Recipe: %s   Seed: %d\n"
		+ "A/D or Left/Right to move, Space/Up/W to jump\n"
		+ "Collision is UNVERIFIED - see this project's README.md"
	) % [result.get("recipe_id", "?"), int(payload.get("seed", -1))]
